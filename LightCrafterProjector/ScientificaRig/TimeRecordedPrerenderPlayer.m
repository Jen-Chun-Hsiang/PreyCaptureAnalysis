classdef TimeRecordedPrerenderPlayer < stage.core.Player
    ...
    methods
        ...
        function obj = TimeRecordedPrerenderPlayer(presentation, time_thread)
            obj = obj@stage.core.Player(presentation);
            obj.timeThread = time_thread;
        end
        ...
        function info = replay(obj, canvas)
            ...
            info.start_time = toc(obj.timeThread);
            flip_times = nan(nFrames, 1);
            for frame = 1:nFrames
                canvas.clear();
                ...
                canvas.window.flip();
                flip_times(frame) = toc(obj.timeThread);
                flipTimer.tick();

                canvas.window.pollEvents();
            end
            info.end_time = toc(obj.timeThread);
            info.flip_times = flip_times;
