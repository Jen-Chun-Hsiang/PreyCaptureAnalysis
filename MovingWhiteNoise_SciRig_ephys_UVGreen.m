function OUT = MovingWhiteNoise_SciRig_ephys_UVGreen(IN,OLED,DIO)
...
for b = 1:nBlock
    ...
    if RepeatBlockIds(b)
        ...
        for i = 1:nFrame
            ...
            cgloadarray(2, nCols, nRows,[zero_stim_int(:),zero_stim_int(:),Stim_int(:)*IN.GreenAttenuation], OLED.width, OLED.height);
            cgdrawsprite(1,MovingSteps_rp(i, 1)+x1,MovingSteps_rp(i, 2)+y1, 1)
            cgdrawsprite(2,MovingSteps_rp(i, 1)+x2,MovingSteps_rp(i, 2)+y2, 0.5)
        ...
        for i = 1:nFrame
            ...
            cgloadarray(1, nCols, nRows,[zero_stim_int(:),Stim_int(:),zero_stim_int(:)], OLED.width, OLED.height);
            cgloadarray(2, nCols, nRows,[zero_stim_int(:),zero_stim_int(:),Stim_int(:)*IN.GreenAttenuation], OLED.width, OLED.height);
            cgdrawsprite(1,MovingSteps(b, i, 1)+x1,MovingSteps(b, i, 2)+y1, 1)
            cgdrawsprite(2,MovingSteps(b, i, 1)+x2,MovingSteps(b, i, 2)+y2, 0.5)
            while toc < 1/IN.NoiseFz - 0.5*1/60
            end   
            FlipTime = cgflip;            
