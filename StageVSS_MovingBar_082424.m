function [OUT] = StageVSS_MovingBar_082424(IN, PROJ, DIO)
%%
...
[X, Y, Z] = meshgrid(width_ids, speed_ids, direc_ids);
Stims = [X(:) Y(:) Z(:)];
nStim = size(Stims, 1);
%%
...
presentation_flash.setBackgroundColor([0, 0, contrast_bg]);
...
for b = 1:IN.repeat
    ...
    for i = 1:nStim
        ...
        presentation_movspot.setBackgroundColor([0, 0, contrast_bg]);
        ...
        DataTable(ii, :) = [0, stim_ids(1), stim_ids(2), stim_ids(3), nan];
