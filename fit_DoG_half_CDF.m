...
%
% MODEL:
%   y = gain * (half_cdf(s, mu_c, sigma_c) + w_s * half_cdf(s, mu_s, sigma_s)) + global_bias
%   
...
%              .mu_c (center mean), .sigma_c (center std)
%              .mu_s (surround mean), .sigma_s (surround std)
%              .w_s (surround weight, negative), .gain, .global_bias
%   fit_stats- structure with fitting statistics (.fval, .exitflag, .output)
%
% CONSTRAINTS:
%   - mu_c: can be slightly negative to zero
%   - mu_s: zero or positive
...
if length(s) < 6
    error('Need at least 6 valid data points to fit 6 parameters');
end
...
% Parameters: [mu_c, sigma_c, mu_s, sigma_s, w_s, gain, global_bias]
model_fun = @(params, x) params(6) * (...
    arrayfun(@(xi) half_cdf(xi, params(1), params(2)), x) + ...
    params(5) * arrayfun(@(xi) half_cdf(xi, params(3), params(4)), x) ...
    ) + params(7);
...
% Initial guesses
mu_c_init = s_mid * 0.3;           % center mean (slightly left of middle)
sigma_c_init = s_range * 0.15;     % center std (narrow)
mu_s_init = s_mid * 0.7;           % surround mean (right of center)
sigma_s_init = s_range * 0.4;      % surround std (broader)
w_s_init = -0.3;                   % surround weight (negative)
gain_init = y_range / 0.5;         % initial gain estimate
bias_init = y_baseline;            % baseline offset

p0 = [mu_c_init, sigma_c_init, mu_s_init, sigma_s_init, w_s_init, gain_init, bias_init];

% Parameter bounds
% [mu_c, sigma_c, mu_s, sigma_s, w_s, gain, global_bias]
lb = [-s_range*0.2, s_range*0.05, 0, s_range*0.1, -1, 0.01, -inf];
ub = [s_range*0.2, s_range*0.5, s_range*2, s_range*2, 0, inf, inf];

% Nonlinear constraints: sigma_s > sigma_c
nonlcon = @(params) deal([], params(2) - params(4));  % sigma_c - sigma_s <= 0
...
p_struct = struct();
p_struct.mu_c = p_opt(1);
p_struct.sigma_c = p_opt(2);
p_struct.mu_s = p_opt(3);
p_struct.sigma_s = p_opt(4);
p_struct.w_s = p_opt(5);
p_struct.gain = p_opt(6);
p_struct.global_bias = p_opt(7);

% Generate smooth fitted curve
s_min = min(s) - s_range * 0.1;
